...
class Expression():
    ...
    def integration(self, var):
        if isint(self) or isnumber(self):
            return self * var
        if type(self) == Variable and self == var:
            return (self**Constant(2)) / Constant(2)
        if type(self) == Variable and self != var:
            return self * var
        if self.op_symbol == "+":
            return self.lhs.integration(var) + self.rhs.integration(var)
        if self.op_symbol == "-":
            return self.lhs.integration(var) - self.rhs.integration(var)
        if self.op_symbol == '*':
            x = self.lhs * self.rhs
            print(x)
            return x.lhs.integration(var)
    ...
    def evaluate(self, Dic={}):
        ...
        else:  # for now we assume that this means self is a tree
            if self.op_symbol == '+':
